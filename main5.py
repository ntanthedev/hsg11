...
def crawl(sbd):
    ...
    while True:
        try:
            t = time.time() * 1000
            rs = requests.Session()
            rs.get(f"https://hatinh.edu.vn/tracuudiemthihsg",verify=True)
            data = io.BytesIO(rs.get(
                f"https://hatinh.edu.vn/api/Common/Captcha/getCaptcha?returnType=image&site=32982&width=150&height=50&t={t}").content)
            ...
            data = {
                ...
                'itemId': '61e9eacee138231c262880a2',
            ...
            response = rs.post('https://hatinh.edu.vn/', params=params, data=data)
